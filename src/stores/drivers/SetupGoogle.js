...
var _steps = [{
  name: 'check',
  title: 'Checking Google Compute Engine',
  message: 'Kitematic is checking your GCE credentials. Please make sure your token is registered.',
  totalPercent: 35,
  percent: 0,
  seconds: 60,
  run: Promise.coroutine(function* (progressCallback) {
    var GOOGLE_AUTH_TOKEN = localStorage.getItem('settings.google-auth-token');
    var options = {
  ...
  run: Promise.coroutine(function* (progressCallback) {
    ...
    if (!exists || (yield machine.state()) === 'Error') {
      ...
      yield machine.create(NAME, ["--google-auth-token", GOOGLE_AUTH_TOKEN]);
      return;
    }
